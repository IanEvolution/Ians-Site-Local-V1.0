...
test.afterAll(async () => {
  let log = '';
  // Collect all tested URLs (from both passed and failed arrays)
  const allUrls = Array.from(new Set([...passed.map(e => e.url), ...failed.map(e => e.url)]));
  for (const url of allUrls) {
    // Find title and pass/fail for this URL
    const passEntry = passed.find(e => e.url === url);
    const failEntry = failed.find(e => e.url === url);
    const title = passEntry ? passEntry.title : (failEntry ? failEntry.title : '');
    const titleStatus = passEntry ? 'passed' : 'failed';
    log += `${url}\n`;
    log += `  Title: ${titleStatus} (${title})\n`;
    if (linkResults[url]) {
      log += `  Links passed:\n`;
      for (const link of linkResults[url].passed) {
        log += `    [PASS] ${link.text} -> ${link.href}\n`;
      }
      log += `  Links failed:\n`;
      for (const link of linkResults[url].failed) {
